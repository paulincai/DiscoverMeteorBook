/* globals OAuth, recreateError, Random, mixpanel */
...
Accounts.validateNewUser(function (user) {
  if (user.services && user.services['meteor-developer']) {
    const email = user.services['meteor-developer'].emails[0].address
...
const _retrievePendingCredential = function (key, credentialSecret) {
  check(key, String)
...
const resetTokenAttributes = function (email) {
...
Meteor.users.mutate = {
  insert: function (doc) {
    ...
    let user = Meteor.users.findOne({ 'services.meteor-developer.emails.address': doc.email })
    let userId = user._id
    if (!user) {
      userId = Accounts.createUser({ email: doc.email })
      user = Meteor.users.findOne(userId)
    }

    const attrs = resetTokenAttributes(doc.email)
  ...
  resetPassword: function (email) {
    ...
    if (user) {
      const attrs = resetTokenAttributes(email)
      Meteor.users.update(user._id, { $set: attrs })
      const url = user.resetPasswordUrl(attrs['services.password.reset'].token)
      Email.send({
...
Meteor.methods({
  ...
  'Meteor.users.mutate.insertAndPurchase': function (email, productKey, levelKey) {
    ...
    const userId = Meteor.users.mutate.insert({ email })
    user = Meteor.users.findOne(userId)
  ...
  tokenUsed: function (token) {
    const user = Meteor.users.findOne({ originalToken: token })
  ...
  oauthUserExists: function (credentialToken, credentialSecret) {
    ...
    const result = _retrievePendingCredential(credentialToken, credentialSecret)
  ...
  attachMDAccountToUserByCredential: function (token, credentialToken, credentialSecret) {
    ...
    const updated = Meteor.users.update({ 'services.password.reset.token': token },
  ...
  attachMDAccountToCurrentUser: function (credentialToken, credentialSecret) {
    ...
    const result = OAuth._retrievePendingCredential(credentialToken, credentialSecret)

    if (!result) {
      throw new Meteor.Error('Oauth failure')
    }

    Meteor.users.update(this.userId, { $set: { 'services.meteor-developer': result.serviceData } })
  }
})
