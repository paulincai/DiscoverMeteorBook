/* globals MailChimpAPIObject */
import { Users } from '../imports/api/collections/users'
import { logError } from '../lib/helpers'

const MailChimpListId = 'ed0fb4ac61'

// note: we're not using the product key for now
const subscribeUser = (user, productKey, levelKey) => {
    ...
    if (error) {
      if (error.code === 214) {
        ...
        MailChimpAPIObject.listUpdateMember({
          id: MailChimpListId,
          email_address: user.getEmail(user).replace('+', '%2B'),
          merge_vars: {
...
Meteor.methods({
  ...
  getInterestGroups: function () {
    ...
    MailChimpAPIObject.listInterestGroupings({ id: MailChimpListId }, (error, data) => {
      if (error) {
        console.log('Error!')
