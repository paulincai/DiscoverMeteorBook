/* globals Migrations, Billing */
import { Meteor } from 'meteor/meteor'
import { Purchases } from '../../api/collections/purchases'
import { Products } from '../../api/collections/products'
import { Sites } from '../../api/collections/sites'
import { Users } from '../../api/collections/users'
import { Gumroad } from '../../../server/gumroad'

const onePerRecord = (cursor, name, fn) => {
  if (!_.isFunction(fn)) {
    throw new Error('Must pass function into `onePerRecord`')
  }
...
const onePerRecordParallel = (cursor, name, fn) => {
  if (!_.isFunction(fn)) {
    throw new Error('Must pass function into `onePerRecordParallel`')
  }
...
Migrations.add({
  ...
  up: function() {
    ...
    onePerRecord(mdUsers, 'user', function(mdUser) {
      const email = mdUser.getEmail()
      const passwordsUser = Meteor.users.findOne({
        'emails.address': email
      })
      if (passwordsUser) {
...
Migrations.add({
  ...
  up: function () {
    ...
    const dmBookId = Products.mutate.insert({
      siteKey: 'dm',
      key: 'dmBook',
  ...
  up: function() {
    ...
    onePerRecord(Meteor.users.find({purchases: {$exists: true}}), 'user', function(user) {
      ...
      if (!userFailed) {
        ...
        const newPurchase = Billing.createPurchase(user, dmBook, toLevelKey)
...
Migrations.add({
  ...
  version: 6,
  up: function () {
    const usersWithNoEmail = Meteor.users.find({ email: { $exists: false } })
...
Migrations.add({
  ...
  version: 7,
  up: function () {
    const purchasesWithNoEmail = Purchases.find({ email: { $exists: false } })
